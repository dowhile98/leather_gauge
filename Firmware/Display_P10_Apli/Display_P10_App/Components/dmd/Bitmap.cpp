    ...
    : scr_width(width), scr_height(height), scr_stride((width + 7) / 8), frame_buffer(0), _font(0), textColor(White)
{
    ...
    frame_buffer = (uint8_t *)dmd_malloc(size);
    if (frame_buffer)
...
{
    uint8_t bitmap_w = *((uint8_t *)bitmap);
    uint8_t bitmap_s = (bitmap_w + 7) >> 3;
    uint8_t bitmap_h = *((uint8_t *)(bitmap) + 1);
    uint8_t invColor = !color;
    for (uint8_t by = 0; by < bitmap_h; ++by)
    {
...
#define Font_IsFixed(font) (*((font)) == 0 && \
                            *((font) + 1) == 0)
#define Font_getWidth(font) (*((font) + 2))
#define Font_getHeight(font) (*((font) + 3))
#define Font_getFirstChar(font) (*((font) + 4))
#define Font_getCharCount(font) (*((font) + 5))
...
int Bitmap::drawChar(int x, int y, char ch)
{
    ...
    else
    {
        ...
        for (uint8_t temp = 0; temp < char_index; ++temp)
        {
            ...
            image += *(_font + 6 + temp) * heightBytes;
        }
    }
    ...
    for (uint8_t cx = 0; cx < char_width; ++cx)
    {
        ...
        {
            uint8_t value = *(image + cy * char_width + cx);
            int posn;
...
int Bitmap::getCharWidth(char letter) const
{
    ...
    if (Font_IsFixed(_font))
        ...
        return *(_font + 6 + (index - first_char));
}
...
{
    uint8_t bitmap_w = *((uint8_t *)pattern);
    uint8_t bitmap_s = (bitmap_w + 7) >> 3;
    uint8_t bitmap_h = *((uint8_t *)(pattern) + 1);
