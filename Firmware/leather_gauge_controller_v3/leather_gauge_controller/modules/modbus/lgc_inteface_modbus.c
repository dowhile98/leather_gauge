...
#include "usart.h"
#include "lwrb.h"
#include "lgc_modbus_task.h" /* For async FSM signaling */
...
/* ============================================================================
 * ASYNC API IMPLEMENTATION (for lgc_modbus_task FSM)
 * ============================================================================ */
...
error_t lgc_modbus_send_raw_pdu(uint8_t function_code, const uint8_t *pdu, uint16_t pdu_len)
{
	...
	nmbs_error err = nmbs_send_raw_pdu(&nmbs, function_code, pdu, pdu_len);

	return (err == NMBS_ERROR_NONE) ? NO_ERROR : ERROR_FAILURE;
}
...
{
	if (pdu_out == NULL || max_len == 0)
	{
		return ERROR_INVALID_PARAMETER;
	}

	nmbs_error err = nmbs_receive_raw_pdu_response(&nmbs, pdu_out, max_len);
...
static void lgc_modbus_rx_callback(UART_HandleTypeDef *huart, uint16_t Pos)
{
	...
	/* Release semaphore to signal data availability (legacy) */
	osReleaseSemaphore(&modbus_rx_semaphore);

	/* Signal async Modbus FSM that RX is complete */
	LgcModbusTask_SignalRxComplete();
}
