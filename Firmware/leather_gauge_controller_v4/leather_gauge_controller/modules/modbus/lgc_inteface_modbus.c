...
static LGC_BUS_MODE_t current_bus_mode = LGC_BUS_MODE_MODBUS;
static uint8_t burst_buffer[MODBUS_RX_BUFFER_SIZE];
static uint16_t burst_len = 0;
static uint16_t burst_rx_index = 0;
...
error_t lgc_interface_modbus_set_mode(LGC_BUS_MODE_t mode)
{
    ...
    if (mode == LGC_BUS_MODE_DAISY_CHAIN)
    {
        lwrb_reset(&modbus_rx_rb);
        burst_rx_index = 0;
...
{
    if (buffer == NULL || len == NULL) return ERROR_FAILURE;
    
    osAcquireMutex(&mutex);
    memcpy(buffer, burst_buffer, burst_len);
    *len = burst_len;
    osReleaseMutex(&mutex);
...
static void lgc_modbus_rx_callback(UART_HandleTypeDef *huart, uint16_t Pos)
{
	if (current_bus_mode == LGC_BUS_MODE_MODBUS)
	{
	...
	{
		/* Daisy Chain Mode: Accumulate data into burst buffer */
		if (burst_rx_index + Pos <= MODBUS_RX_BUFFER_SIZE)
		{
			memcpy(&burst_buffer[burst_rx_index], modbus_rx_temp_buffer, Pos);
			burst_rx_index += Pos;
            
            /* Expected burst length: 11 sensors * 3 bytes each = 33 bytes */
            if (burst_rx_index >= 33)
            {
                burst_len = burst_rx_index;
                burst_rx_index = 0;
                /* Notify the system that a burst is ready for processing */
                osSetEventBits(&events, LGC_EVENT_BURST_READY);
            }
		}
        else
        {
