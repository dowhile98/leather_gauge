...
void lgc_hmi_update_task_entry(void *param)
{
	...
	for (;;)
	{
		...
		case HMI_PAGE1:
		{
			...
			//->guard
			dwin_write_vp_u16(&dwin_hmi, LGC_HMI_VP_STATE, live_status.guard_status ? 0 : 1);
			//->speed
			dwin_write_vp_u16(&dwin_hmi, LGC_HMI_VP_ICON_SPEEP, live_status.motor_speed);
			// set date (YYYY / MM / DD)
	...
	for (;;)
	{
		...
		case LGC_HMI_TOUCH_PAGE_ADDR:
		{
			...
			else if (value == HMI_PAGE7)
			{
				...
				dwin_write_vp_u16(&dwin_hmi, LGC_HMI_VP_CONFIG_DAY, datetime.day);
				dwin_write_vp_u16(&dwin_hmi, LGC_HMI_VP_CONFIG_MONTH, datetime.month);
				dwin_write_vp_u16(&dwin_hmi, LGC_HMI_VP_CONFIG_YEAR, datetime.year - RTC_BASE_YEAR);
			}
		...
		case LGC_HMI_VP_CONFIG_SAVE_CMD:
		{
			...
			{
				// get day
				dwin_read_u16(&dwin_hmi, LGC_HMI_VP_CONFIG_DAY, &value, 1000);
				osAcquireMutex(&hmi_data.mutex);
				hmi_data.day = value;
				osReleaseMutex(&hmi_data.mutex);
				// get month
				dwin_read_u16(&dwin_hmi, LGC_HMI_VP_CONFIG_MONTH, &value, 1000);
				osAcquireMutex(&hmi_data.mutex);
				hmi_data.month = value;
				osReleaseMutex(&hmi_data.mutex);
				// get year
				dwin_read_u16(&dwin_hmi, LGC_HMI_VP_CONFIG_YEAR, &value, 1000);
				osAcquireMutex(&hmi_data.mutex);
				hmi_data.year = value;
				osReleaseMutex(&hmi_data.mutex);
				// set date time
