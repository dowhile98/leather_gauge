...
- **RTOS** para gestión de tareas concurrentes (Azure ThreadX)
- **Interfaz LwPKT Cascade** para comunicación rápida con sensores remotos (⚡ 550ms vs 2s Modbus legacy)
...
2. **Lectura de Sensores:** Lee los 11 sensores mediante interfaz **LwPKT cascade mode** (1 broadcast → 11 respuestas secuenciales, ⚡ ~550ms)
...
- **Protocolo:** **LwPKT (Lightweight Packet Protocol)** vía RS-485 (115200 baud)
- **Modo de Operación:** **Cascade Broadcast** (optimizado para baja latencia)
  - 1 comando broadcast (FLAGS=1) → 11 respuestas secuenciales
  - Cada sensor responde cuando FLAGS == su dirección (1-11)
  - Sensor 11 responde con FLAGS=0 (fin de cascada)
- **Tipo de Dato Recibido:**
  - **Payload:** uint16_t (2 bytes) - bitmask digital directo
  - **Formato:** bits 0-9 = estado de 10 fotodiodos (1=cuero detectado, 0=vacío)
  - **Nota:** El sensor hace la conversión analógica→digital internamente (threshold configurable en sensor)
- **Latencia Total:** ~550ms para 11 sensores (vs ~2s en Modbus legacy)
- **Timeout por Sensor:** 100ms
- **Validación CRC:** CRC-8 obligatoria en cada paquete

### Flujo del Algoritmo de Medición

#### **PASO 1: Lectura de Sensores (Síncrona con Encoder)**

Código: `domain/use_cases/lgc_uc_measure_area.c` (Clean Architecture)

```c
// En cada pulso del encoder:
// 1. Lectura rápida mediante LwPKT cascade (ISensorReader interface)
...
if (res != ERR_OK) {
    // Manejar timeout o error de CRC
    return res;
}

// 2. Extraer bitmask de cada sensor
// El sensor ya envía el bitmask digital directamente (sin conversión)
for (uint8_t i = 0; i < LGC_SENSOR_NUMBER; i++) {
    data.sensor[i] = sensor_data.sensors[i].status;  // uint16_t bitmask
    // bits 0-9: estado de cada fotodiodo (1=cuero detectado, 0=vacío)
    // El sensor hace threshold internamente antes de enviar
}
```

**Flujo Interno del Adapter LwPKT:**

```c
// adapters/communication/lwpkt_adapter/lgc_lwpkt_adapter.c

// 1. Enviar broadcast (FLAGS=1)
lwpkt_write(&adapter->lwpkt, 0xFF, CMD_READ_CASCADE, 1, NULL, 0);

// 2. Recibir 11 paquetes secuencialmente
for (uint8_t sensor_idx = 0; sensor_idx < 11; sensor_idx++) {
    // Esperar respuesta (timeout 100ms)
    tx_semaphore_get(&adapter->rx_sem, TX_MS_TO_TICKS(100));

    // 3. Parsear payload: uint16_t bitmask directo (2 bytes)
    if (adapter->lwpkt.data_len == 2) {
        // El sensor ya envía el bitmask digital procesado
        uint16_t bitmask = *((uint16_t *)adapter->lwpkt.data);
        out_data->sensors[sensor_idx].status = bitmask;
        // bits 0-9: estado de fotodiodos (el sensor aplicó threshold internamente)
    }
}
```
...
- **LwPKT Cascade:** ~550ms (11 sensores)
- **Modbus Legacy:** ~2000ms (deprecated)
- **Mejora:** ⚡ **73% más rápido**

**Salida:** Arreglo `data.sensor[0..10]` con valores uint16_t bitmask de cada sensor
...
```mermaid
graph TD
    A["Esperar Pulso Encoder<br/>(osWaitForSemaphore)"] --> B["Leer 11 Sensores<br/>(LwPKT Cascade ~550ms)"]
...
- **Sensores LwPKT:** 11 unidades (direcciones 1-11, modo cascade)
- **Payload Recibido:** uint16_t (2 bytes) - bitmask digital directo
- **Datos:** bits 0-9 = estado de 10 fotodiodos (1=detectado, 0=vacío)
