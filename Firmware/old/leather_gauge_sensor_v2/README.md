...
## 🖥️ Testing desde PC (Hardware Validation)

### Conversor RS-485 USB

Para probar el firmware con sensores reales desde una computadora, utiliza el script Python incluido.
...
# Conectar conversor RS-485 USB (CH340, FTDI, etc.)
# Linux: /dev/ttyUSB0
# Windows: COM3
...
```bash
# 1. Lectura Individual (modo tradicional)
python main.py --port /dev/ttyUSB0 --mode individual --sensor 1

# 2. Lectura en Cascada (modo optimizado, 67% más rápido)
python main.py --port /dev/ttyUSB0 --mode cascade --sensors 11

# 3. Comparar latencias (Individual vs Cascada)
python main.py --port /dev/ttyUSB0 --mode compare --sensors 11

# 4. Lectura RAW ADC (diagnóstico)
python main.py --port /dev/ttyUSB0 --mode raw --sensor 1

# 5. Modo Interactivo (comandos manuales)
python main.py --port /dev/ttyUSB0 --mode interactive
```

**Performance Esperado:**

| Modo       | Latencia (11 sensores) | Mejora         |
| ---------- | ---------------------- | -------------- |
| Individual | ~1.5s                  | -              |
| Cascada    | ~500ms                 | 67% más rápido |

**Troubleshooting:**

- **Timeout:** Verificar cables A-B, alimentación 12-24VDC, baudrate 115200
- **Permission denied (Linux):** `sudo usermod -a -G dialout $USER`
- **CRC errors:** Agregar terminación 120Ω en extremos del bus RS-485
- **Respuestas duplicadas:** Conflicto de direcciones, verificar config de cada sensor

📘 **Documentación completa:** Ver [`TESTING_PC.md`](TESTING_PC.md)
