...
typedef enum
{
	...
	LGC_HMI_VP_FEEDBACK_MOTOR = 0x1112, // Feedback del motor (ON/OFF)
	...
	LGC_HMI_VP_CONFIG_SAVE_CMD = 0x1002,	// Comando de guardar (config o sensor)
	LGC_HMI_VP_CONFIG_SAVE_RESULT = 0x1003, // Resultado de guardar (0=idle,1=ok,2=fail)
...
/**
 * @brief  Check if HMI sensor test mode is active
 * @note   Used by Modbus FSM to yield bus control during sensor testing
 * @return true if sensor test is active, false otherwise
 */
bool lgc_hmi_is_sensor_test_active(void);
