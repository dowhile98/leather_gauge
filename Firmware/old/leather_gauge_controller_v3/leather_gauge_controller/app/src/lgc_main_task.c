...
#include "lgc_i_sensor_cache.h" /* Async sensor data access */
//-------------------------------------------------------------------------------
// defines
//-------------------------------------------------------------------------------
...
/* Sensor cache interface (async architecture) */
static ILgcSensorCache_t *s_sensor_cache = NULL;
...
void lgc_main_task_entry(void *param)
{
	...
	/* Get sensor cache interface for async reads */
	s_sensor_cache = lgc_get_sensor_cache_interface();
	...
	for (;;)
	{
		...
		case LGC_RUNNING:
		{
			...
			{
				/* ============================================================
				 * ASYNC ARCHITECTURE: Read from sensor cache (non-blocking)
				 * The Modbus task updates the cache asynchronously.
				 * ============================================================ */

				last_sensor_read_ms = osGetSystemTime();

				/* Trigger next Modbus read cycle (for next encoder pulse) */
				lgc_trigger_modbus_cycle();

				/* Read sensor data from cache (instant, no blocking) */
				if (s_sensor_cache != NULL)
				{
					LgcSensorArray_t sensor_array;
					error_t cache_err = s_sensor_cache->get_all_sensors(
						s_sensor_cache->context, &sensor_array);

					if (cache_err == NO_ERROR)
					{
						/* Copy data to legacy structure for compatibility */
						for (uint8_t i = 0; i < LGC_SENSOR_NUMBER; i++)
						{
							data.sensor[i] = sensor_array.sensors[i].value;
						}
						data.sensor_status = sensor_array.status_bitmask;
						sensor_read_time_diff_ms = sensor_array.read_cycle_ms;

						/* Track sequence for change detection */
						s_last_sequence = sensor_array.sequence_number;
					}
					else
					{
						/* Cache read error - mark all sensors as faulty */
						data.sensor_status = 0x07FF; /* All 11 bits set */
					}
				}
				else
				{
					/* FATAL: Sensor cache not initialized - system misconfiguration */
					data.sensor_status = 0x07FF; /* All 11 bits set (all sensors faulty) */
				}
